...
func _on_add_terrain_pressed() -> void:
	...
	if popup.accepted:
		...
		undo_manager.add_do_method(self, &"perform_add_terrain", popup.terrain_name, popup.terrain_color, popup.terrain_type, popup.terrain_categories, popup.terrain_decorations, {path = popup.terrain_icon})
...
func _on_edit_terrain_pressed() -> void:
	...
	if popup.accepted:
		...
		undo_manager.add_do_method(self, &"perform_edit_terrain", selected_entry, popup.terrain_name, popup.terrain_color, popup.terrain_type, popup.terrain_categories, popup.terrain_decorations, {path = popup.terrain_icon})
		undo_manager.add_undo_method(self, &"perform_edit_terrain", selected_entry, t.name, t.color, t.type, t.categories, t.decorations, t.icon)
...
func _on_remove_terrain_pressed() -> void:
	...
	if confirmed[0]:
		...
		undo_manager.add_undo_method(self, &"perform_add_terrain", t.name, t.color, t.type, t.categories, t.decorations, t.icon)
...
func perform_add_terrain(name: String, color: Color, type: int, categories: Array, decorations: Array, icon:Dictionary = {}) -> void:
	if BetterTerrain.add_terrain(tileset, name, color, type, categories, decorations, icon):
...
func perform_edit_terrain(index: int, name: String, color: Color, type: int, categories: Array, decorations: Array, icon: Dictionary = {}) -> void:
	if index >= terrain_list.get_child_count():
		return
	...
	if BetterTerrain.set_terrain(tileset, index, name, color, type, categories, decorations, valid_icon):
